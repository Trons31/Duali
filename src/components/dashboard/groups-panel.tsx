...
import { FiChevronDown, FiChevronRight, FiLayers, FiPlus, FiSearch, FiUsers } from "react-icons/fi";
  ...
  return (
    <div className="-mx-4 -my-6 min-h-[calc(100vh-5rem)] bg-[#f8fafc] px-4 py-6 sm:-mx-6 sm:-my-8 sm:px-6 sm:py-8">
                ...
                >
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="truncate text-[15px] font-semibold text-ink-950">{group.nombre}</h3>
                      <p className="mt-1 truncate text-xs font-medium text-ink-500">
                        {group._count.students} {group._count.students === 1 ? "alumno" : "alumnos"}
                        {" · "}
                        {currency(estimateGroupMonthlyIncome(group))}
                      </p>
                    </div>
      ...
      <Modal
        ...
        title={detailGroup?.nombre ?? ""}
        description={detailGroup?.descripcion || "Sin descripción registrada."}
        ...
        {detailGroup ? (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              <GroupInfoBlock
                label="Alumnos"
                value={detailGroup._count.students}
                helper={`${countActiveStudents(detailGroup)} activos`}
              />
              <GroupInfoBlock label="Cobros" value={detailGroup._count.monthlyPayments} helper="registrados" />
              <GroupInfoBlock
                className="col-span-2"
                label="Ingreso estimado"
                value={currency(estimateGroupMonthlyIncome(detailGroup))}
                helper="mensual"
              />
            </div>
            ...
            <Button
              ...
              onClick={() => router.push(`/dashboard/grupos/${detailGroup.id}`)}
            >
